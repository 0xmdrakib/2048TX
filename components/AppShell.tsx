        ...
        {leaderboardErr ? (
          <div className="mt-3 rounded-2xl border border-[var(--cardBorder)] bg-[var(--card)] p-3 text-sm">
            ...
            <div className="mt-1 text-[11px] opacity-70">{leaderboardErr}</div>
                ...
                {address ? (
          <div className="mt-3 flex items-center justify-between gap-3 rounded-2xl border border-[var(--cardBorder)] bg-[var(--card)] px-4 py-3">
            <div className="min-w-0">
              <div className="text-xs opacity-70">Your wallet</div>
              <div className="truncate font-mono text-xs">{shorten(address)}</div>
            </div>
            <div className="text-right">
              <div className="text-xs opacity-70">Your rank</div>
              <div className="text-base font-extrabold">
                {meRankInfo ? `#${meRankInfo.rank}` : "Not in Top 100"}
              </div>
            </div>
          </div>
        ) : null}
...
<div className="mt-3 max-h-[60vh] space-y-2 overflow-auto">
          {(leaderboard ?? []).map((e, i) => {
            ...
            <div
              key={e.address}
              className={`flex items-center justify-between rounded-2xl border border-[var(--cardBorder)] bg-[var(--card)] px-3 py-2 ${isMe ? "bg-black/5 ring-1 ring-black/10" : ""}`}
              title={e.address}
            >
